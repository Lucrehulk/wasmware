...
export default async function rewrite_js_with_imports() {
    ...
    let assign_re = new RegExp(`\\b${imports_var}\\s*=`);
    let assign_match = assign_re.exec(js);
    ...
    function find_matching(start) {
        ...
        for (let i = start; i < js.length; i++) {
            ...
            if (in_comment) {
                if (in_comment == '//' && c == '\n') in_comment = null;
                else if (in_comment == '/*' && c == '*' && n == '/') { in_comment = null; i++; }
                continue;
            }
            if (!in_str) {
                if (c == '/' && n == '/') { in_comment = '//'; i++; continue; }
                if (c == '/' && n == '*') { in_comment = '/*'; i++; continue; }
            }
            if (!in_str && (c == '"' || c == '\'' || c == '`')) { in_str = c; continue; }
            if (in_str) {
                if (c == '\\') { i++; continue; }
        ...
        throw new Error("Unterminated imports structure");
    }
    ...
    if (!is_root_array) {
        let key_re = new RegExp(`(?:['\"]${module_name}['\"]|\\b${module_name}\\b)\\s*:`);
        let slice = js.slice(root_start, root_end);
        let key_match = key_re.exec(slice);
        ...
        for (let i = root_start + 1; i < root_end; i++) {
            ...
            if (js[i] == '{' || js[i] == '[') { first_entry = i; break; }
        }
